...
const sections = [
    ...
    {
        name: 'CMS',
        courses: ["cms-ceremony", "cms-workshop_learn2study", "ws", "snack", "cms-pr", "cms-pg", "cms-al", "cms-ga", "cms-an"],
    },
    {
        name: 'Génie civil',
        courses: ["ceremony", "workshop_learn2study", "ws", "snack", "pm", "al", "pr", "an", "ch"],
...
const COURSES = {
    ...
    "snack":{
        "name": "Goûter",
        "breaks": ["ven 30 15h30-16h", "sam 31 15h30-16h", "lun 2 15h30-16h", "mar 3 15h30-16h", "mer 4 15h30-16h"],
    },
    "cms-ceremony": {
        "name": "Cérémonie d'ouverture",
...
function findCourse(section, search_day, search_time) {
    for (let c in section.courses) {
        const course = COURSES[section.courses[c]];
        if (findTimeSlot(course.courses, search_day, search_time)) {
            return {
                name: course.name,
                code: section.courses[c].split("-").reverse()[0],
                type: "c"
            };
        }
        if (findTimeSlot(course.exercices, search_day, search_time)) {
            return {
                name: course.name,
                code: section.courses[c].split("-").reverse()[0],
                type: "e"
        ...
        if (findTimeSlot(course.presentations, search_day, search_time)) {
            return {
                name: course.name,
                code: section.courses[c].split("-").reverse()[0],
                type: "p"
        ...
        if (findTimeSlot(course.activities, search_day, search_time)) {
            return {
                name: course.name,
                code: section.courses[c].split("-").reverse()[0],
                type: "w"
        ...
        if (findTimeSlot(course.breaks, search_day, search_time)) {
            return {
                name: course.name,
                code: section.courses[c].split("-").reverse()[0],
                type: "b"
